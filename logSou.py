...
path="C:/Dev/southern/samples/log/"

execfile("ConnectToSouthern.py")


#echotoken = "f0cde7c1cd644e42807f8a950bd70814"
def save_portal(echotoken):
    
    q1= query_portal("select * from gateway_log_new where echo_token='%s' allow filtering;" %(echotoken))
...
def save_pss(echotoken):
    ...
    q1= query_adapter("select * from pss_log_new where transactionid='%s' allow filtering;" %(echotoken))
...
import subprocess
# subprocess.Popen(r"explorer /select,'"+path+sys.argv[1]+"'")
subprocess.call('explorer "C:\Dev\southern\samples\log\\'+ sys.argv[1]+'\\"', shell=True)
